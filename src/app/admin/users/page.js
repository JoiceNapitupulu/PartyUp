...
export default function AdminUsers() {
    ...
    const handleResetAllData = () => {
        ...
        setUsers(usersData);
        localStorage.setItem("usersList", JSON.stringify(usersData));
        addLog("[SYSTEM] DIRECTORY RESET TO DEFAULT DATA DUMMY.");
    };

    const handleToggleBan = (e, userId, userName) => {
        e.stopPropagation();
    ...
    const handleImpersonate = (e, userObj) => {
        e.stopPropagation();
        localStorage.setItem("currentUser", JSON.stringify(userObj));
        if (typeof triggerAuthChange === "function") {
            triggerAuthChange();
        }
        ...
        <div className="flex-grow p-6 md:p-8 flex flex-col gap-6">
            <div className="flex justify-between items-center border-b-2 border-retro-black pb-4">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="font-pixel text-[7px] bg-gray-200 hover:bg-gray-300 border border-retro-black px-2 py-1 cursor-pointer"
                    >
                        RESET DATA
                    </button>
            ...
            <div className="bg-white border-4 border-retro-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10">
                    {users.map((item) => {
                        const isAdminUser = item.role?.toLowerCase() === "admin" || item.user_id === "USR-000";

                        return (
                            <div
                                key={item.user_id}
                                className={`relative z-10 p-4 border-4 flex flex-col justify-between gap-4 transition-all ${isAdminUser
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between border-t border-b border-retro-black/10 py-2.5 relative z-20">
                                    ...
                                    {isAdminUser ? (
                                        ...
                                        <select
                                            ...
                                            className="font-sans text-[10px] p-1 border-2 border-retro-black bg-white cursor-pointer focus:outline-none disabled:cursor-not-allowed relative z-30"
                                        >
                                ...
                                <div className="flex gap-2 relative z-20">
                                    ...
                                    {!isAdminUser && (
                                        <button
                                            type="button"
                                            onClick={(e) => handleToggleBan(e, item.user_id, item.name)}
