...
export default function ProjectCard({ project, showAuthor = true, onApply }) {
  ...
  return (
    <>
      {/* 1. KARTU UTAMA QUEST BOARD */}
      <div
        onClick={() => setIsDetailOpen(true)}
        className={`bg-white pixel-border pixel-shadow p-5 flex flex-col justify-between gap-4 transition-transform hover:-translate-y-1 cursor-pointer select-none ${
          project.isVerified ? "border-amber-400" : ""
        } ${isClosed ? "opacity-75" : ""}`}
      >
        <div>
          {/* Category Header + Lencana Verifikasi Emas */}
          <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-pixel text-[9px] px-2 py-1 bg-navy-blue text-white pixel-border-sm">
                {project.category}
              </span>
              {/* Lencana Verifikasi Emas */}
              {project.isVerified && (
                <span className="font-pixel text-[8px] bg-yellow-400 text-retro-black px-1.5 py-1 border border-retro-black shadow-sm animate-pulse font-bold">
                  ★ VERIFIED
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {project.looking_for.map((skill, index) => {
                ...
                return (
                  <span
                    key={index}
                    className={`flex items-center gap-1.5 font-pixel text-[8px] px-2 py-1 pixel-border-sm ${
                      isRole
      ...
      {isDetailOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-retro-black/60 p-4 backdrop-blur-sm">
          <div className="bg-retro-bg pixel-border pixel-shadow w-full max-w-lg p-6 flex flex-col gap-4 relative max-h-[95vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            {/* Informasi Utama + Lencana Verifikasi Emas */}
            <div className="space-y-4 text-left">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {/* Lencana Emas Verifikasi di Dalam Modal */}
                  {project.isVerified && (
                    <span className="font-pixel text-[8px] bg-yellow-400 text-retro-black px-1.5 py-0.5 border border-retro-black animate-pulse font-bold">
